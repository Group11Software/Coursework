...
public class App {
    ...
    public void generateTopNPopulatedCountriesReport1(String continent, int n) {
    ...
    public void generateTopNPopulatedCountriesInRegion1(String region, int n) {
        ArrayList<Country> countries = new ArrayList<>();
        try {
            Statement stmt = con.createStatement();
            String sql = String.format(
    ...
    public void generateTopNPopulatedCitiesInRegion(String region, int n) {
        ArrayList<City> cities = new ArrayList<>();
        try {
            Statement stmt = con.createStatement();
            String sql = String.format(
            ...
            // Gather data for the top N cities
            while (rset.next()) {
                long id = rset.getLong("ID");
                String name = rset.getString("Name");
                String countryCode = rset.getString("CountryCode");
            ...
            // Save report to a file
            StringBuilder report = new StringBuilder();
            report.append(String.format("Top %d Most Populated Cities in %s:\n", n, region));
            report.append(String.format("%-30s %-15s %-20s %-15s\n", "City", "Country Code", "District", "Population"));
            report.append("=".repeat(80)).append("\n"); // Separator line

            for (City city : cities) {
                report.append(String.format("%-30s %-15s %-20s %-15d\n",
                        city.getName(), city.getCountryCode(), city.getDistrict(), city.getPopulation()));
            }

            new File("./output/").mkdir();
            BufferedWriter writer = new BufferedWriter(
                    new FileWriter("./output/Top" + n + "PopulatedCities_" + region + ".txt"));
            writer.write(report.toString());
            writer.close();
            ...
            System.out.println("Error generating top N populated cities report: " + e.getMessage());
        }
    }

    public void generateTopNPopulatedCitiesInCountry(String countryCode, int n) {
        ArrayList<City> cities = new ArrayList<>();
        try {
            Statement stmt = con.createStatement();
            String sql = String.format(
            ...
            while (rset.next()) {
                long id = rset.getLong("ID");
                String name = rset.getString("Name");
                String countryCodeDb = rset.getString("CountryCode");
                String district = rset.getString("District");
                long population = rset.getLong("Population");

                City city = new City(id, name, countryCodeDb, district, population);
                cities.add(city);
            }
            ...
            new File("./output/").mkdir();
            BufferedWriter writer = new BufferedWriter(
                    new FileWriter("./output/Top" + n + "PopulatedCities_" + countryCode + ".txt"));
            writer.write(report.toString());
            writer.close();
    ...
    public ArrayList<City> getKyotoDistrictCitiesSortedByPopulation1() {
        ArrayList<City> kyotoCities = new ArrayList<>();
        try {
            Statement stmt = con.createStatement();
            String sql = "SELECT * FROM city " +
            ...
            System.out.println("Executing SQL: " + sql); // Print the SQL query to the terminal for debugging
            ResultSet rset = stmt.executeQuery(sql);
            ...
            System.out.println("Error fetching cities in Kyoto district: " + e.getMessage());
            e.printStackTrace(); // Print stack trace to get more details of the error
        }
        return kyotoCities;
    }

    public void generateTopNDistrictCitiesReport(int n) {
        ArrayList<City> kyotoCities = getKyotoDistrictCitiesSortedByPopulation();
        if (kyotoCities == null || kyotoCities.isEmpty()) {
            System.out.println("No cities found in Kyoto district.");
            return;
        ...
        // Limit to the top N cities
        ArrayList<City> topNCities = new ArrayList<>(kyotoCities.subList(0, Math.min(n, kyotoCities.size())));

        // Print to the terminal
        System.out.println("Top " + n + " Cities in Kyoto District Sorted by Population (Greatest to Lowest):");
